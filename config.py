...
conf.registerChannelValue(
    ...
    registry.Boolean(False, """Determines whether or not the bot will
                            automatically kickban guilty users when they join
                            the channel."""))
conf.registerChannelValue(
    GuiltByAssociation,
    'expire',
    registry.Integer(0, """Duration (in seconds) to wait before lifting the
                        ban. 0 to disable."""))
conf.registerChannelValue(
    GuiltByAssociation,
    'chans',
    registry.String('', """The list of channels (separated by comma) which will
                        result in an automatic kickban if found in a user's
                        /whois output."""))
